...
apt-get update
apt-get install -y gcc make bzip2 ffmpeg unzip git wget dos2unix sox libsox-dev libsox-fmt-all
...
if [ -f $USRDIR/.setup_complete ]; then
    ...
    conda create -y -n spl
    source activate spl
    conda install -y pytorch torchvision -c soumith
    #conda install pytorch torchvision cuda80 -c soumith
    conda install -y scipy scikit-learn jupyter matplotlib h5py requests
    pip install librosa bs4
